...
void Con_CompleteCommand( field_t *field )
{
	...
	else
	{
		...
		qsort( con.cmds, con.matchCount, sizeof( char* ), (_CoreCrtNonSecureSearchSortCompareFunction)Con_SortCmds );
		...
		Cmd_LookupCmds( NULL, NULL, (setpair_t)Con_PrintMatches );
		Cvar_LookupVars( 0, NULL, NULL, (setpair_t)Con_PrintMatches );
	}
}
/*
=========
...
qboolean Cmd_CheckMapsList_R( qboolean fRefresh, qboolean onlyingamedir )
{
	...
	buffer = (char*)Mem_Alloc( host.mempool, t->numfilenames * 2 * sizeof( result ));
...
{
	Cvar_LookupVars( CVAR_ARCHIVE, NULL, f, (setpair_t)Cmd_WriteCvar );
}

void Cmd_WriteServerVariables( file_t *f )
{
	FS_Printf( f, "// servernotify cvars\n" );
	Cvar_LookupVars( CVAR_SERVERNOTIFY, NULL, f, (setpair_t)Cmd_WriteServerCvar );
}

void Cmd_WriteOpenGLVariables( file_t *f )
{
	Cvar_LookupVars( CVAR_GLCONFIG, NULL, f, (setpair_t)Cmd_WriteOpenGLCvar );
}

void Cmd_WriteRenderVariables( file_t *f )
{
	Cvar_LookupVars( CVAR_RENDERINFO, NULL, f, (setpair_t)Cmd_WriteRenderCvar );
}
