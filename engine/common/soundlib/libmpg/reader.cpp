...
{
	buffy_t	*newbuf = (buffy_t *)malloc( sizeof( buffy_t ));

	if( newbuf == NULL )
		return NULL;

	newbuf->realsize = size > minsize ? size : minsize;
	newbuf->data = (byte*)malloc( newbuf->realsize );
...
static int default_init( mpg123_handle_t *fr )
{
	...
	fr->rdat.read = fr->rdat.r_read != NULL ? fr->rdat.r_read : (mpg_ssize_t(__cdecl*)(int, void*, size_t))read;
