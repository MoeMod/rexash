...
int frame_buffers( mpg123_handle_t *fr )
{
	...
	fr->short_buffs[0][0] = (short*)aligned_pointer( fr->rawbuffs, 16 );
	...
	fr->float_buffs[0][0] = (float*)aligned_pointer( fr->rawbuffs, 16 );
	...
	if( fr->layerscratch == NULL )
	{
		...
		fr->layerscratch = (float*)malloc( scratchsize + 63 );
		if(fr->layerscratch == NULL) return -1;

		// get aligned part of the memory, then divide it up.
		scratcher = (float*)aligned_pointer( fr->layerscratch, 64 );
...
int frame_outbuffer( mpg123_handle_t *fr )
{
	...
	fr->buffer.data = (byte*)aligned_pointer( fr->buffer.rdata, 16 );
	fr->own_buffer = TRUE;
	fr->buffer.fill = 0;
