...
void Image_ConvertPalTo24bit( rgbdata_t *pic )
{
	...
	pal24 = converted = (byte*)Mem_Alloc( host.imagepool, 768 );
	pal32 = pic->palette;
...
void Image_CopyPalette32bit( void )
{
	...
	image.palette = (byte*)Mem_Alloc( host.imagepool, 1024 );
...
byte *Image_FlipInternal( const byte *in, word *srcwidth, word *srcheight, int type, int flags )
{
	...
	case PF_RGBA_32:
	case PF_BGRA_32:
		image.tempbuffer = (byte*)Mem_Realloc( host.imagepool, image.tempbuffer, width * height * samples );
		break;
	default:
...
byte *Image_CreateLumaInternal( byte *fin, int width, int height, int type, int flags )
{
	...
	case PF_INDEXED_24:
	case PF_INDEXED_32:
		out = image.tempbuffer = (byte*)Mem_Realloc( host.imagepool, image.tempbuffer, width * height );
...
qboolean Image_Decompress( const byte *data )
{
	...
	image.tempbuffer = (byte*)Mem_Realloc( host.imagepool, image.tempbuffer, size );
