...
static void stringlistappend( stringlist_t *list, const char *text )
{
	...
	if( list->numstrings >= list->maxstrings )
	{
		...
		list->strings = (char**)Mem_Alloc( fs_mempool, list->maxstrings * sizeof( *list->strings ));
		if( list->numstrings ) Q_memcpy( list->strings, oldstrings, list->numstrings * sizeof( *list->strings ));
		Z_Free( oldstrings );
	}

	textlen = Q_strlen( text ) + 1;
	list->strings[list->numstrings] = (char*)Mem_Alloc( fs_mempool, textlen );
...
static qboolean FS_ParseGameInfo( const char *gamedir, gameinfo_t *GameInfo )
{
	...
	if( host.rodir[0] )
	{
		...
		{
			auto afile_ro = FS_LoadDirectFile( filepath_ro, NULL );
			...
			if( afile_ro )
			{
				...
				FS_ParseGenericGameInfo( &gi, (char*)afile_ro, true );
...
dll_user_t *FS_FindLibrary( const char *dllname, qboolean directpath )
{
	...
	hInst = (dll_user_t*)Mem_Alloc( host.mempool, sizeof( dll_user_t ));
...
search_t *FS_Search( const char *pattern, int caseinsensitive, int gamedironly )
{
	...
	basepath = (char*)Mem_Alloc( fs_mempool, basepathlength + 1 );
	...
	if( resultlist.numstrings )
	{
		...
		search = (search_t*)Mem_Alloc( fs_mempool, sizeof(search_t) + numchars + numfiles * sizeof( char* ));
