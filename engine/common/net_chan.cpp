...
void NetSplit_SendLong( netsrc_t sock, size_t length, void *data, netadr_t to, unsigned int maxpacket, unsigned int id, qboolean compress )
{
	...
	if( compress )
		Huff_CompressData( (byte*)data, &length );
...
void Netchan_OutOfBand( int net_socket, netadr_t adr, int length, byte *data )
{
	...
	{
		// send the datagram
		NET_SendPacket( (netsrc_t)net_socket, BF_GetNumBytesWritten( &send ), BF_GetData( &send ), adr );
	}
}
...
qboolean Netchan_CopyFileFragments( netchan_t *chan, sizebuf_t *msg )
{
	...
	buffer = (byte*)Mem_Alloc( net_mempool, nsize + 1 );
	p = chan->incomingbufs[ FRAG_FILE_STREAM ];
	pos = 0;
