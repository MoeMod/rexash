...
static void Mod_LoadSubmodels( const dlump_t *l )
{
	...
	if( world.loading )
		world.draw_surfaces = (msurface_t * *)Mem_Alloc( loadmodel->mempool, world.max_surfaces * sizeof( msurface_t* ));
}
...
static void Mod_LoadTextures( const dlump_t *l )
{
	...
	for( i = 0; i < loadmodel->numtextures; i++ )
	{
		...
		if( *(offset + i + 1) == -1 )
		{
			...
			tx = (texture_t *)Mem_Alloc( loadmodel->mempool, sizeof( *tx ));
...
static void Mod_BuildPolygon( mextrasurf_t *info, msurface_t *surf, int numVerts, const float *verts )
{
	...
	buffer = (byte *)Mem_Alloc( loadmodel->mempool, bufSize );
...
static void Mod_LoadSurfaces( const dlump_t *l )
{
	...
	loadmodel->surfaces = (msurface_t*)Mem_Alloc( loadmodel->mempool, count * sizeof( msurface_t ));
	loadmodel->cache.data = (mextrasurf_t*)Mem_Alloc( loadmodel->mempool, count * sizeof( mextrasurf_t ));
	out = loadmodel->surfaces;
	info = (mextrasurf_t *)loadmodel->cache.data;
...
static void Mod_LoadVertexes( const dlump_t *l )
{
	...
	out = loadmodel->vertexes = (mvertex_t*)Mem_Alloc( loadmodel->mempool, count * sizeof( mvertex_t ));
...
static void Mod_LoadEdges( const dlump_t *l )
{
	...
	count = l->filelen / sizeof( *in );
	loadmodel->edges = out = (medge_t*)Mem_Alloc( loadmodel->mempool, count * sizeof( medge_t ));
...
static void Mod_LoadSurfEdges( const dlump_t *l )
{
	...
	loadmodel->surfedges = out = (dsurfedge_t*)Mem_Alloc( loadmodel->mempool, count * sizeof( dsurfedge_t ));
...
static void Mod_LoadMarkSurfaces( const dlump_t *l )
{
	...
	count = l->filelen / sizeof( *in );
	loadmodel->marksurfaces = out = (msurface_t **)Mem_Alloc( loadmodel->mempool, count * sizeof( *out ));
...
static void Mod_LoadEntities( const dlump_t *l )
{
	...
	loadmodel->entities = (char*)Mem_Alloc( loadmodel->mempool, l->filelen + 1 );
...
static void Mod_LoadClipnodes31( const dlump_t *l, const dlump_t *l2, const dlump_t *l3 )
{
	...
	out2 = (dclipnode_t*)Mem_Alloc( loadmodel->mempool, count2 * sizeof( *out2 ));
	...
	out3 = (dclipnode_t*)Mem_Alloc( loadmodel->mempool, count3 * sizeof( *out3 ));
...
void Mod_CalcPHS( void )
{
	...
	visofs = (int*)Mem_Alloc( worldmodel->mempool, num * sizeof( int ));
	uncompressed_vis = (byte*)Mem_Alloc( worldmodel->mempool, rowbytes * num * 2 );
	uncompressed_pas = uncompressed_vis + rowbytes * num;
	compressed_pas = (byte*)Mem_Alloc( worldmodel->mempool, phsdatasize );
	...
	compressed_pas = (byte*)Mem_Realloc( worldmodel->mempool, compressed_pas, total_size );
