...
size_t Sequence_GetLine( char *line, int lineMaxLen )
{
	...
	char* read = (char*)Q_strchr( g_scan, '\n' );
...
void Sequence_CreateDefaultsCommand( sequenceEntry_s *entry )
{
	...
	cmd = (sequenceCommandLine_s * )Z_Malloc( sizeof( sequenceCommandLine_s ) );
...
char Sequence_ParseEntry( void )
{
	...
	entry = (sequenceEntry_s * )Z_Malloc( sizeof( sequenceEntry_s ) );
...
sentenceGroupEntry_s *Sequence_AddSentenceGroup( char *groupName )
{
	...
	entry                = (sentenceGroupEntry_s * )Z_Malloc( sizeof( sentenceGroupEntry_s ) );
...
void Sequence_AddSentenceToGroup( char *groupName, char *data )
{
	...
	entry            = (sentenceEntry_s * )Z_Malloc( sizeof( sentenceEntry_s ) );
	entry->nextEntry = NULL;
...
sequenceCommandLine_s *Sequence_CopyCommand( sequenceCommandLine_s *commandOrig )
{
	...
	commandCopy = (sequenceCommandLine_s *)Z_Malloc( sizeof( sequenceCommandLine_s ) );
