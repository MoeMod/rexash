...
static void R_StudioDrawPoints_legacy( void )
{
	...
	if( r_studio_sort_textures->integer )
	{
		...
		qsort( g_sortedMeshes, m_pSubModel->nummesh, sizeof( sortedmesh_t ), (_CoreCrtNonSecureSearchSortCompareFunction)R_StudioMeshCompare );
	}
...
static void R_StudioLoadTexture( model_t *mod, studiohdr_t *phdr, mstudiotexture_t *ptexture )
{
	...
	if( !Q_strnicmp( ptexture->name, "DM_Base", 7 ) || !Q_strnicmp( ptexture->name, "remap", 5 ))
	{
		...
		tx = (texture_t *)Mem_Alloc( mod->mempool, sizeof( *tx ) + size );
		mod->textures[i] = tx;
...
static engine_studio_api_t gStudioAPI =
{
	Mod_Calloc,
	Mod_CacheCheck,
	(void(__cdecl*)(char*,cache_user_s*))Mod_LoadCacheFile,
	(model_s* (__cdecl*)(const char*,int))Mod_ForName,
	Mod_Extradata,
	Mod_Handle,
	pfnGetCurrentEntity,
	pfnPlayerInfo,
