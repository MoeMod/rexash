...
static byte *R_SpriteLoadFrame( model_t *mod, byte *pin, mspriteframe_t **ppframe, int num )
{
	...
	pspriteframe = (mspriteframe_t*)Mem_Alloc(mod->mempool, sizeof(mspriteframe_t));
...
static byte *R_SpriteLoadGroup( model_t *mod, byte *pin, mspriteframe_t **ppframe, int framenum )
{
	...
	pspritegroup = (mspritegroup_t * )Mem_Alloc( mod->mempool, groupsize );
...
void Mod_LoadSpriteModel( model_t *mod, byte *buffer, qboolean *loaded, uint texFlags )
{
	...
	psprite = (msprite_t *)Mem_Alloc( mod->mempool, size );
...
void Mod_LoadMapSprite( model_t *mod, const void *buffer, size_t size, qboolean *loaded )
{
	...
	pix = FS_LoadImage( texname, (byte*)buffer, size );
	...
	mod->mempool = Mem_AllocPool( va( "^2%s^7", mod->name ));
	psprite = (msprite_t *)Mem_Alloc( mod->mempool, sizeof( msprite_t ) + ( numframes - 1 ) * sizeof( psprite->frames ));
	mod->cache.data = psprite;	// make link to extradata
	...
	for( i = xl = yl = 0; i < numframes; i++ )
	{
		...
		psprite->frames[i].frameptr = (mspriteframe_t*)Mem_Alloc( mod->mempool, sizeof( mspriteframe_t ));
