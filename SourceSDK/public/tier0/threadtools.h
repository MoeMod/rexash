...
public:
	...
	operator const void *() const          					{ return (T *)CThreadLocalBase::Get(); }
	operator void *()                      					{ return (T *)CThreadLocalBase::Get(); }

	operator const T *() const							    { return (T *)CThreadLocalBase::Get(); }
	operator const T *()          							{ return (T *)CThreadLocalBase::Get(); }
	operator T *()											{ return (T *)CThreadLocalBase::Get(); }

	int			operator=( int i )							{ AssertMsg( i == 0, "Only NULL allowed on integer assign" ); CThreadLocalBase::Set( NULL ); return 0; }
	T *			operator=( T *p )							{ CThreadLocalBase::Set( p ); return p; }

	bool        operator !() const							{ return (!CThreadLocalBase::Get()); }
	bool        operator!=( int i ) const					{ AssertMsg( i == 0, "Only NULL allowed on integer compare" ); return (this->Get() != NULL); }
	bool        operator==( int i ) const					{ AssertMsg( i == 0, "Only NULL allowed on integer compare" ); return (this->Get() == NULL); }
	bool		operator==( const void *p ) const			{ return (CThreadLocalBase::Get() == p); }
	bool		operator!=( const void *p ) const			{ return (CThreadLocalBase::Get() != p); }
	bool		operator==( const T *p ) const				{ return operator==((void*)p); }
	bool		operator!=( const T *p ) const				{ return operator!=((void*)p); }

	T *  		operator->()								{ return (T *)CThreadLocalBase::Get(); }
	T &  		operator *()								{ return *((T *)CThreadLocalBase::Get()); }

	const T *   operator->() const							{ return (T *)CThreadLocalBase::Get(); }
	const T &   operator *() const							{ return *((T *)CThreadLocalBase::Get()); }

	const T &	operator[]( int i ) const					{ return *((T *)CThreadLocalBase::Get() + i); }
	T &			operator[]( int i )							{ return *((T *)CThreadLocalBase::Get() + i); }
