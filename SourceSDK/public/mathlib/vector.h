...
FORCEINLINE void VectorCopy(const float *src, float *dst)
{
	dst[0] = src[0];
	dst[1] = src[1];
	dst[2] = src[2];
...
FORCEINLINE void VectorCopy(const Vector &src, float *dst)
{
	CHECK_VALID(src);
	dst[0] = src.x;
	dst[1] = src.y;
...
FORCEINLINE void VectorCopy(const float *src, Vector &dst)
{
	CHECK_VALID(dst);
	dst.x = src[0];
	dst.y = src[1];
...
FORCEINLINE void VectorAdd(const Vector &a, const float *b, Vector &c)
{
	CHECK_VALID(a);
	CHECK_VALID(c);
	c.x = a.x + b[0];
...
inline void VectorScale(const float *in, float scale, float *result)
{
	result[0] = in[0] * scale;
	result[1] = in[1] * scale;
	result[2] = in[2] * scale;
