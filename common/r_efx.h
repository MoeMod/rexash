...
struct efx_api_s
{
	...
	void		(*R_FireField) 				(const float* org, int radius, int modelIndex, int count, int flags, float life);
	...
	void		(*R_UserTracerParticle)		(const float* org, const float* vel, float life, int colorIndex, float length, unsigned char deathcontext, void (*deathfunc)(struct particle_s* particle));
	particle_t* (*R_TracerParticles)		(const float* org, const float* vel, float life);
	...
	TEMPENTITY* (*R_TempSprite)				(const float* pos, const float* dir, float scale, int modelIndex, int rendermode, int renderfx, float a, float life, int flags);
	...
	BEAM*		(*R_BeamCirclePoints)		(int type, const float* start, const float* end, int modelIndex, float life, float width, float amplitude, float brightness, float speed, int startFrame, float framerate, float r, float g, float b);
	...
	BEAM*		(*R_BeamLightning)			(const float* start, const float* end, int modelIndex, float life, float width, float amplitude, float brightness, float speed);
	BEAM*		(*R_BeamPoints)				(const float* start, const float* end, int modelIndex, float life, float width, float amplitude, float brightness, float speed, int startFrame, float framerate, float r, float g, float b);
	...
	void		(*R_FireCustomDecal)		(int textureIndex, int entity, int modelIndex, const float* position, int flags, float scale);
};
